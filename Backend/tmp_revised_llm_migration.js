...
async function migrate() {
  ...
  try {
    // 1. Drop existing LLM tables
    console.log('Dropping existing LLM tables...');
    ...
    // 2. Create Tables with User's Schema
    console.log('Creating LLM tables based on latest request...');
    ...
    await connection.query(`
      CREATE TABLE llm_dissertation (
        id INT AUTO_INCREMENT PRIMARY KEY,
        regulation_id INT NOT NULL,
        title VARCHAR(255) DEFAULT 'Dissertation',
    ...
    await connection.query(`
      CREATE TABLE llm_specializations (
        id INT AUTO_INCREMENT PRIMARY KEY,
        regulation_id INT NOT NULL,
        specialization_key VARCHAR(50) NOT NULL,
    ...
    // 3. Seed Data for 2023-24 (using new field names)
    ...
    for (const p of compulsory) {
      await connection.execute(
        'INSERT INTO llm_compulsory_papers (regulation_id, paper_code, paper_title, credits, display_order) VALUES (?, ?, ?, ?, ?)',
        [regId, ...p]
      );
    }
    ...
    // Dissertation
    await connection.execute(
      'INSERT INTO llm_dissertation (regulation_id, title, credits, description) VALUES (?, ?, ?, ?)',
    ...
    for (const [s_key, title, ord, papers] of specs) {
      const [sResult] = await connection.execute(
        'INSERT INTO llm_specializations (regulation_id, specialization_key, specialization_title, display_order) VALUES (?, ?, ?, ?)',
        [regId, s_key, title, ord]
      );
      const specId = sResult.insertId;
      for (const p of papers) {
        await connection.execute(
          'INSERT INTO llm_specialization_papers (specialization_id, paper_code, paper_title, credits, display_order) VALUES (?, ?, ?, ?, ?)',
          [specId, ...p]
        );
      }
    }
